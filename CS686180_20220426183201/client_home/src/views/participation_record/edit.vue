<template>
	<div class="diy_edit page_participation_record" id="participation_record_edit">
		<div class='warp'>
			<div class='container'>
				<div class='row diy_edit_content_box'>
					<div v-if="$check_field('set','title_name') || $check_field('add','title_name') || $check_field('get','title_name')" class="form-item col-12 col-md-6">
						...
						<div class="diy_field diy_text">
							...
							<span v-else-if="$check_field('get','title_name')">{{ form['student_name'] }}</span>
						</div>
					</div>
					<div v-if="$check_field('set','working_hours') || $check_field('add','working_hours') || $check_field('get','working_hours')" class="form-item col-12 col-md-6">
						<div class="diy_title">
							<span>劳动时间:
							</span>
						</div>
						...
						<div class="diy_field diy_date">
							<input type="date" :disabled="disabledObj['working_hours_isDisabled']" id="form_working_hours" v-model="form['working_hours']" placeholder="请输入劳动时间" v-if="(form['working_hours'] && $check_field('set','working_hours')) || (!form['working_hours'] && $check_field('add','working_hours'))" />
							<span v-else-if="$check_field('get','working_hours')">{{ form['student_name'] }}</span>
						</div>
					</div>
	...
	export default {
		...
		data() {
			return {
				...
				obj: {
					"title_name":'', // 标题名称
					"working_hours": new Date().getTime(),
		...
		methods: {
			...
			change_file(files, str) {
				...
				this.$post("~/api/participation_record/upload?", form, (res) => {
					if (res.result) {
        ...
        if (JSON.stringify(this.form["working_hours"]).indexOf("-")===-1) {
