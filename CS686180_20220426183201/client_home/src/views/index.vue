		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/volunteer_activities/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_volunteer_activities = !show_list_volunteer_activities">
		                        <div class="switch_box">
		                            <span v-if="show_list_volunteer_activities"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--志愿者活动推荐列表组件 -->
		                <list_volunteer_activities v-if="show_list_volunteer_activities" :list="list_volunteer_activities" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_volunteer_activities.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_volunteer_activities" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_volunteer_activities" :key="i" @click="to_details('volunteer_activities',o,'volunteer_activities_id')">
		                            <td v-for="(oj,n) in fields_volunteer_activities">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_volunteer_activities from "@/components/diy/list_volunteer_activities.vue";
	...
	export default {
		...
		data() {
			return {
				...
				show_list_volunteer_activities: true,
				list_volunteer_activities: [],
				vm_arr_volunteer_activities: [
		...
		methods: {
			...
			// 获取志愿者活动列表
			get_volunteer_activities() {
				let url = "~/api/volunteer_activities/get_list?";
				// url = "~/api/volunteer_activities/get_list?orderby=hits desc";
				this.$get(url, {
					"orderby": "hits desc",
					"page": 1,
					"size": 8
					...
					if (json.result) {
						this.list_volunteer_activities = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_volunteer_activities() {
		        var list = this.list_volunteer_activities;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_volunteer_activities.map((o) => {
		                // 第二个中括号是对象的属性
